...
macro_rules! heterogeneous_tuple_impl {
    ...
    ($($parser_param: ident)*) => {
        ...
        // Generic impl over all data and parser types
            ...
            fn sequence(self) -> impl Parser<Output = Self::OutputsTuple> {
                ...
                $crate::parser::parser(
                    move |input, mut context| {
                        ...
                        // Run each parser in turn, putting the output in a new variable with the same name as the parser type
                        $(
                            let (input, $parser_param) = $parser_param.parse(input, context.borrow())?;
                            let $parser_param = res.take_errors_from($parser_param);
                        )*

                        // Combine the variables into a parser output
